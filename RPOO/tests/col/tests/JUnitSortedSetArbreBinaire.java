...
import col.SortedSetArbreBinaire;
import sdr.correction.ArbreBinaire;
import sdr.correction.ChaineSimple;

//------------------------ Classe dérivée dédiée aux tests

class SortedSetArbreBinaireTEST<E extends Comparable<E>> extends SortedSetArbreBinaire<E> {
	@SafeVarargs
	SortedSetArbreBinaireTEST(E... val) {
		super();
		if (val.length > 0) {
			this.arbre = new ArbreBinaire<>(val[0]);
			for (int i=1; i<val.length; i++)
				this.arbre.ajoute(val[i]);
			this.size = val.length;
		}
	}

	ArbreBinaire<E> getArbre() {
		return this.arbre;
	}
...
public class JUnitSortedSetArbreBinaire {

	// ------------------------ Données de test

	protected SortedSetArbreBinaireTEST<Integer> s1, s2, s3, s4;
	...
	@SafeVarargs
	private static <E extends Comparable<E>> boolean valideSortedSetArbreBinaire(SortedSetArbreBinaireTEST<E> s, E... v) {
		ArbreBinaire<E> a = s.getArbre();
		int size = s.getSize();
		// On traite le cas de l'ensemble vide :
		if (a == null || size == 0) { // ensemble supposé vide
			if (a != null || size != 0)
				return ko("attributs incohérents : size : " + size + " ; arbre : " + a);
		...
		ChaineSimple<E> varbre = a.infixe();
		for (ChaineSimple<E> c = varbre; c != null; c = c.suivant, nbc++) {
			E val = c.valeur;
			r += " " + val;
			ko = ko || val == null || !Arrays.stream(v).anyMatch(val::equals);
			for (ChaineSimple<E> d = varbre; !ko && d != c; d = d.suivant)
	...
	@Before
	public void setup() {
		this.s1 = new SortedSetArbreBinaireTEST<>();
		this.s2 = new SortedSetArbreBinaireTEST<>(1);
		this.s3 = new SortedSetArbreBinaireTEST<>(1,2);
		this.s4 = new SortedSetArbreBinaireTEST<>(5,7,8,2,1);
	}

	@Test
	public void testSetup() { // vérification de l'état des données de test
		assertTrue(valideSortedSetArbreBinaire(this.s1));
		assertTrue(valideSortedSetArbreBinaire(this.s2, 1));
		assertTrue(valideSortedSetArbreBinaire(this.s3, 1, 2));
		assertTrue(valideSortedSetArbreBinaire(this.s4, 5, 7, 8, 2, 1));
	}
	...
	public void testClear() {
		...
		assertTrue(valideSortedSetArbreBinaire(this.s1));
		this.s2.clear();
		assertTrue(valideSortedSetArbreBinaire(this.s2));
		this.s3.clear();
		assertTrue(valideSortedSetArbreBinaire(this.s3));
		this.s4.clear();
		assertTrue(valideSortedSetArbreBinaire(this.s4));
	}

	@Test
	public void testAdd() {
		assertTrue(this.s1.add(1));
		assertTrue(valideSortedSetArbreBinaire(this.s1, 1));
		assertFalse(this.s1.add(1));
		assertTrue(valideSortedSetArbreBinaire(this.s1, 1));
		assertTrue(this.s1.add(2));
		assertTrue(valideSortedSetArbreBinaire(this.s1, 1, 2));
		assertFalse(this.s1.add(2));
		assertTrue(valideSortedSetArbreBinaire(this.s1, 1, 2));
		assertFalse(this.s4.add(8));
		assertTrue(valideSortedSetArbreBinaire(this.s4, 5, 7, 8, 2, 1));
		assertTrue(this.s4.add(9));
		assertTrue(valideSortedSetArbreBinaire(this.s4, 5, 7, 8, 2, 1, 9));
	...
	public void testRemove() {
		...
		assertTrue(valideSortedSetArbreBinaire(this.s3, 1));
		...
		assertTrue(valideSortedSetArbreBinaire(this.s4, 5, 7, 2, 1));
		assertFalse(this.s4.remove(8));
		assertTrue(valideSortedSetArbreBinaire(this.s4, 5, 7, 2, 1));
		assertTrue(this.s4.remove(1));
		assertTrue(valideSortedSetArbreBinaire(this.s4, 5, 7, 2));
		assertFalse(this.s4.remove(1));
		assertTrue(valideSortedSetArbreBinaire(this.s4, 5, 7, 2));
		assertTrue(this.s4.remove(5));
		assertTrue(valideSortedSetArbreBinaire(this.s4, 7, 2));
	...
	public void testIterator() {
		...
		if (x==2 || x==8) it.remove();
		assertFalse(it.hasNext());
		assertTrue(valideSortedSetArbreBinaire(this.s4, 1, 7, 5));
	}
	...
	public void testContainsAll() {
		...
		this.s3.add(5);
		this.s3.add(7);
		assertTrue(valideSortedSetArbreBinaire(this.s3, 1, 2, 5, 7));
		assertFalse(this.s3.containsAll(this.s4));
		this.s3.add(8);
		assertTrue(valideSortedSetArbreBinaire(this.s3, 1, 2, 5, 7, 8));
		assertTrue(this.s3.containsAll(this.s4));
		this.s4.remove(1);
		assertTrue(valideSortedSetArbreBinaire(this.s4, 5, 7, 8, 2));
		assertTrue(this.s3.containsAll(this.s4));
		this.s4.add(0);
		assertTrue(valideSortedSetArbreBinaire(this.s4, 5, 7, 8, 2, 0));
		assertFalse(this.s3.containsAll(this.s4));
		this.s3.add(0);
		assertTrue(valideSortedSetArbreBinaire(this.s3, 1, 2, 5, 7, 8, 0));
		assertTrue(this.s3.containsAll(this.s4));
	}
	...
	public void testAddAll() {
		...
		assertTrue(valideSortedSetArbreBinaire(this.s3, 5, 7, 8, 2, 1));
		assertFalse(this.s3.addAll(this.s4));
		assertTrue(valideSortedSetArbreBinaire(this.s3, 5, 7, 8, 2, 1));
		assertTrue(this.s1.addAll(this.s4));
		assertTrue(valideSortedSetArbreBinaire(this.s1, 5, 7, 8, 2, 1));
	...
	public void testRetainAll() {
		...
		assertTrue(valideSortedSetArbreBinaire(this.s4, 1, 2));
		assertFalse(this.s4.retainAll(this.s3));
		assertTrue(valideSortedSetArbreBinaire(this.s4, 1, 2));
		assertTrue(this.s4.retainAll(this.s2));
		assertTrue(valideSortedSetArbreBinaire(this.s4, 1));
	...
	public void testRemoveAll() {
		...
		assertTrue(valideSortedSetArbreBinaire(this.s4, 5, 7, 8));		
